 ...
 */
export function setupPracticeArea(practiceContainerId: string): void {
...
export interface PracticeDisplayEntry {
    character: string;
 ...
 */
export function showPracticeEntry(entry: PracticeDisplayEntry | null): void {
    ...
    if (!container || !charEl || !translitEl || !playBtn) return;
    ...
    if (!entry) {
        ...
        const knewBtn = document.getElementById('knew-btn') as HTMLButtonElement | null;
        const didntBtn = document.getElementById('didnt-btn') as HTMLButtonElement | null;
        if (knewBtn) knewBtn.disabled = true;
        if (didntBtn) didntBtn.disabled = true;
        const ans = document.getElementById('answer-input') as HTMLInputElement | null;
        if (ans) ans.value = '';
        return;
    }
    ...
    (playBtn as HTMLButtonElement).disabled = !entry.audio_path;
    (playBtn as HTMLButtonElement).dataset.audio = entry.audio_path || '';
    ...
    if (didntBtn) didntBtn.disabled = false;
    const ans = document.getElementById('answer-input') as HTMLInputElement | null;
    if (ans) ans.value = '';
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (active.id === 'practice-char' && (e.key === 'Enter' || e.key === ' ')) {
        e.preventDefault();
        const playBtn = document.getElementById('play-audio') as HTMLButtonElement | null;
        playBtn?.click();
    }
});
 ...
 */
export function bindPracticeCallbacks(onPlay: (audioPath?: string) => void, onKnown: () => void, onUnknown: () => void): void {
    const playBtn = document.getElementById('play-audio');
    ...
    if (playBtn) {
        playBtn.addEventListener('click', () => {
            const path = (playBtn as HTMLButtonElement).dataset.audio;
            onPlay(path || undefined);
        });
    }
    if (knewBtn) {
        knewBtn.addEventListener('click', () => onKnown());
        if (knewBtn instanceof HTMLButtonElement) knewBtn.disabled = true; // default until an entry is shown
    }
    if (didntBtn) {
        didntBtn.addEventListener('click', () => onUnknown());
        if (didntBtn instanceof HTMLButtonElement) didntBtn.disabled = true;
    }
}
 ...
 */
export function bindAnswerInput(onSubmit: (value: string) => void): void {
 ...
 */
export function flashIncorrect(): void {
    ...
    const originalBorder = (charEl as HTMLElement).style.borderColor;
    try {
        // Web Animations API for a quick shake
        if ((charEl as any).animate) {
            (charEl as any).animate([
            ...
            (charEl as HTMLElement).style.transform = 'translateX(-8px)';
            setTimeout(() => (charEl as HTMLElement).style.transform = 'translateX(8px)', 80);
            setTimeout(() => (charEl as HTMLElement).style.transform = 'translateX(-6px)', 160);
            setTimeout(() => (charEl as HTMLElement).style.transform = 'translateX(6px)', 240);
            setTimeout(() => (charEl as HTMLElement).style.transform = 'translateX(0px)', 320);
        }
    } catch (e) {
        // ignore animation errors
    }

    (charEl as HTMLElement).style.borderColor = '#fecaca'; // red-200
    setTimeout(() => {
        (charEl as HTMLElement).style.borderColor = originalBorder || '';
    }, 600);
}
