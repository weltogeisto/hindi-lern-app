export type GhostMode = 'ghost' | 'off' | 'full';
...
function sample<T>(arr: T[], n: number): T[] { return shuffle(arr).slice(0, Math.min(n, arr.length)); }

export function loadScriptLabState(): LabState {
    try {
        const raw = localStorage.getItem(LAB_KEY);
        return raw ? { ...DEFAULT_LAB_STATE, ...JSON.parse(raw) } as LabState : { ...DEFAULT_LAB_STATE };
    } catch {
...
function registerResult(state: LabState, ok: boolean): void {
...
function registerConfusion(state: LabState, expected: string, picked: string): void {
    const key = `${expected}→${picked}`;
    state.confusions[key] = (state.confusions[key] || 0) + 1;
}

function bestConfusion(state: LabState): { expected: string; picked: string; value: number } | null {
...
function maybeAddConfusionOptions(alphabet: AlphabetEntry[], target: AlphabetEntry): AlphabetEntry[] {
...
function supportLabel(stage: number): string {
    return SUPPORT_LABELS[Math.max(0, Math.min(stage, SUPPORT_LABELS.length - 1))];
}

function supportsHidden(stage: number): boolean {
    return stage >= 1;
}

export function getScriptLabAccuracy(state = loadScriptLabState()): number {
...
function chooseExercise(data: LabData, state: LabState) {
    ...
    if (focus && state.focusCount < 2) {
        ...
        const options = shuffle([target, wrong, ...others].filter(Boolean) as AlphabetEntry[]).slice(0, 4);
        return {
            render(container: HTMLElement, onDone: () => void) {
                container.innerHTML = `
                ...
                const feedback = container.querySelector('#script-lab-feedback') as HTMLElement;
                container.querySelectorAll<HTMLButtonElement>('.script-lab-choice').forEach(btn => {
        ...
        () => {
            ...
            return {
                render(container: HTMLElement, onDone: () => void) {
                    ...
                    const input = container.querySelector('#script-lab-input') as HTMLInputElement;
                    const feedback = container.querySelector('#script-lab-feedback') as HTMLElement;
                    const check = () => {
...
export function renderScriptLab(container: HTMLElement, alphabet: AlphabetEntry[], vocab: VocabEntry[], options: RenderOptions): void {
    opts = options;
    const data: LabData = {
        alphabet,
    ...
    container.querySelectorAll<HTMLButtonElement>('.script-lab-toggle').forEach(btn => {
        btn.addEventListener('click', () => {
            setGhostMode((btn.dataset.ghost as GhostMode) || 'ghost');
    ...
    const exerciseHost = container.querySelector('#script-lab-exercise') as HTMLElement;
