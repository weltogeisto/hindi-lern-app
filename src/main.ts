...
let practiceQueue: AlphabetEntry[] = [];
let currentIndex = -1;
...
function bindUI(): void {
    ...
    if (newMemBtn) newMemBtn.addEventListener('click', () => {
        if (!appState.alphabet.length) {
            showToast('Alphabet data is still loading. Please try again in a moment.');
            return;
        }
        ...
        setTimeout(() => {
            ...
            const mn = document.getElementById('start-memory'); if (mn) mn.focus();
        }, 300);
    });
...
document.addEventListener('mouseover', (e) => {
    ...
    if (!target) return;
    const tile = target.closest('.alphabet-char') as HTMLElement | null;
    if (!tile) return;
...
function ensureMemoryHandler(container: HTMLElement) {
    ...
    container.addEventListener('click', (ev) => {
        ...
        if (revealed.length === 2) {
            ...
            if (a.p.pairId === b.p.pairId) {
                // match
                a.p.matched = true; b.p.matched = true;
                ...
                setTimeout(() => {
                    if (btnA) btnA.classList.remove('revealed');
                    if (btnB) btnB.classList.remove('revealed');
                    a.p.revealed = false; b.p.revealed = false;
                    state.lock = false;
                }, 800);
            }
        }
...
function updateBestMemoryScore(attempts: number) {
...
document.addEventListener('click', (e) => {
    ...
    if (target.id === 'reset-memory-score') {
        ...
        setTimeout(() => {
            ...
            if (cancel) cancel.addEventListener('click', () => modal.hide());
        }, 50);
    }
});
