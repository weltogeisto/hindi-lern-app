...
import {
    ...
    renderScriptLab,
    loadScriptLabState,
    getScriptLabAccuracy
} from './components/scriptLab.js';

type TabId = 'vocab' | 'alphabet' | 'grammar' | 'dashboard';
...
let vocabView: 'study' | 'script-lab' = 'study';
...
function clearLegacyModeState(): void {
    try {
...
function renderApp(): void {
    ...
    if (activeTab === 'grammar') void renderGrammarTab(content);
...
function renderVocabTab(container: HTMLElement): void {
    ...
    container.querySelectorAll<HTMLButtonElement>('.mode-btn[data-mode]').forEach(btn => {
        btn.addEventListener('click', () => {
    ...
    container.querySelectorAll<HTMLButtonElement>('.mode-btn[data-ghost]').forEach(btn => {
        btn.addEventListener('click', () => {
            setGhostMode((btn.dataset.ghost as 'ghost' | 'off' | 'full') || 'ghost');
            renderVocabTab(container);
        });
    });
...
function renderFlashcard(cardArea: HTMLElement, entry: VocabEntry): void {
    ...
    cardArea.querySelector<HTMLButtonElement>('.play-hindi-btn')?.addEventListener('click', () => {
...
function renderTypingCard(cardArea: HTMLElement, entry: VocabEntry): void {
    ...
    cardArea.querySelector<HTMLButtonElement>('.play-hindi-btn-inline')?.addEventListener('click', () => {
        playHindi(entry.hindi);
    });
